...
def read_sht_data(filename: str, filepath: str, data_name: str) -> pd.DataFrame:
    '''
...
def read_sht_df(filename: str, filepath: str, column_names: list, data_names: list = None) -> pd.DataFrame:
    '''
    ...
    if ".sht" in filename.lower():
        ...
        if not os.path.isfile(filepath + filename + ".SHT"):
            raise FileNotFoundError
        res = shtRipper.ripper.read(filepath + filename + ".SHT")
